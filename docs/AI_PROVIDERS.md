...
Provider output is also merged with deterministic coverage. In strict mode this adds high-confidence strong-signal signatures. When `--learn-observed` is explicitly enabled, it also adds deterministic observed-learning coverage so a model omission does not leave a known high-volume path shape unprotected. CodexSDGate drops empty, oversized, or malformed provider matchers, and normalizes plus deconflicts learned rule IDs before writing `runtime/filters.json`, so provider-generated names remain safe for the proxy's `x-altura-filter` response header and malformed optional matchers cannot make a whole reload fail. Use `--disable-strong-coverage` only when evaluating raw provider behavior.
